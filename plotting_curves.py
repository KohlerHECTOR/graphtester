...
for ax, aig in zip(axes.flat, AIGS):
    curves = np.stack([
        np.load(f'dataset-{aig}-all-actions-False-mc-simu-100-seed-{s}-rs-False_curve_undirected.npy')
        for s in range(SEEDS)
    ], axis=0)
    ...
    curves = np.stack([
        np.load(f'dataset-{aig}-all-actions-False-mc-simu-100-seed-{s}-rs-False_curve_directed.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(curves, axis=0)
    n_steps_c = curves.shape[1]
    ...
    a = np.concatenate([
        np.load(f'dataset-{aig}-all-actions-False-mc-simu-100-seed-{s}-rs-False_perm_curves_undirected.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(a, axis=0)
    ...
    a = np.concatenate([
        np.load(f'dataset-{aig}-all-actions-False-mc-simu-100-seed-{s}-rs-False_perm_curves_directed.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(a, axis=0)
...
try:
    _regret = dict(np.load('res/predictor_regret.npz'))
except FileNotFoundError:
    _regret = {}

try:
    _perm_ranks = dict(np.load('res/predictor_perm_ranks.npz', allow_pickle=True))
except FileNotFoundError:
    _perm_ranks = {}
