...
try:
    _pred_mse = dict(np.load('RES-DUMMY-PREDICTORS/predictor_mse.npz'))
...
for ax, aig in zip(axes.flat, AIGS):
    mc = 10 if aig == 'C6288' else 100
    curves = np.stack([
        np.load(f'RES-AIGS/dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_curve_undirected.npy')
        for s in range(SEEDS)
    ], axis=0)
    ...
    curves = np.stack([
        np.load(f'RES-AIGS/dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_curve_directed.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(curves, axis=0)
    n_steps_c = curves.shape[1]
    ...
    a = np.concatenate([
        np.load(f'RES-AIGS/dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_perm_curves_undirected.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(a, axis=0)
    ...
    a = np.concatenate([
        np.load(f'RES-AIGS/dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_perm_curves_directed.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(a, axis=0)
    ...
    curves = np.stack([
        np.load(f'RES-AIGS/w_edges_dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_curve_undirected.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(curves, axis=0)
    n_steps_c = curves.shape[1]
    ...
    curves = np.stack([
        np.load(f'RES-AIGS/w_edges_dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_curve_directed.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(curves, axis=0)
    n_steps_c = curves.shape[1]
    ...
    a = np.concatenate([
        np.load(f'RES-AIGS/w_edges_dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_perm_curves_undirected.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(a, axis=0)
    n_steps = a.shape[1]
    ...
    a = np.concatenate([
        np.load(f'RES-AIGS/w_edges_dataset-{aig}-all-actions-False-mc-simu-{mc}-seed-{s}-rs-False_perm_curves_directed.npy')
        for s in range(SEEDS)
    ], axis=0)
    m = np.mean(a, axis=0)
    n_steps = a.shape[1]
...
fig.savefig('per_aig.pdf', bbox_inches='tight', dpi=300)
plt.clf()

# --- Bar plot of ranking metrics per AIG ---
try:
    _ranks = dict(np.load('RES-DUMMY-PREDICTORS/predictor_ranks.npz'))
except FileNotFoundError:
    _ranks = {}

try:
    _regret = dict(np.load('RES-DUMMY-PREDICTORS/predictor_regret.npz'))
except FileNotFoundError:
    _regret = {}

try:
    _perm_ranks = dict(np.load('RES-DUMMY-PREDICTORS/predictor_perm_ranks.npz', allow_pickle=True))
except FileNotFoundError:
    _perm_ranks = {}
...
if _ranks:
    ...
    fig_bar.savefig('ranking_bars.pdf', bbox_inches='tight', dpi=300)
    plt.clf()