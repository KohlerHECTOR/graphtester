...
def _init_node_labels(
    ...
    List[str]
        The node labels.
    """
    # For directed graphs, the initial "degree" signature must distinguish in
    # and out degrees, otherwise direction information is lost at iteration 0.
    if G.is_directed():
        degree_repr = [
            f"{i}_{o}" for i, o in zip(G.indegree(), G.outdegree())
        ]
    else:
        degree_repr = [str(deg) for deg in G.vs.degree()]
    ...
    if node_attr:
        ...
        if use_degree:
            return [
                deg + ":" + ";".join([str(attr) for attr in attributes])
                for attributes, deg in zip(zip(*attribute_labels), degree_repr)
            ]
        else:
...
def _aggregate_neighborhood(
    ...
    def _nbr_label(src: int, dst: int, side: str) -> str:
        nbr = dst if src == node_idx else src
        if edge_attr:
            eid = G.get_eid(src, dst, directed=directed, error=False)
            edge = G.es[eid]
            prefix = ":".join([str(edge[attr]) for attr in edge_attr])
            return f"{side};{prefix};{node_labels[nbr]}"
        return f"{side};{node_labels[nbr]}"

    if directed:
        # Aggregate predecessors and successors separately so that
        # 1-WL on directed graphs distinguishes in- and out-neighborhoods.
        in_labels = sorted(
            _nbr_label(src, node_idx, "p") for src in G.predecessors(node_idx)
        )
        out_labels = sorted(
            _nbr_label(node_idx, dst, "s") for dst in G.successors(node_idx)
        )
        label_list = [f"-1;{node_labels[node_idx]}"] + in_labels + out_labels
    else:
        nbr_labels = sorted(
            _nbr_label(min(node_idx, nbr), max(node_idx, nbr), "u")
            for nbr in G.neighbors(node_idx)
        )
        label_list = [f"-1;{node_labels[node_idx]}"] + nbr_labels
