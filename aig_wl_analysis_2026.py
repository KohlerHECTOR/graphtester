...
def load_raw_aigs(aig: str, indices: list[int]):
...
#         iterations=WL_ITERS,
#     )
#     curve_aig = ev.as_dataframe()["Lower Bound MSE"].to_numpy()/y_var
#     np.save(f'{aig}_curve.npy', curve_aig)
...
for _ in range(REPETS):
    ...
    for aig in available:
        _, _, y = load(aig)
        indices = np.random.choice(N_PER_AIG, size=N_SUB, replace=False)
        raw = load_raw_aigs(aig, list(indices))
    ...
    curves.append(ev_agg.as_dataframe()['Lower Bound MSE'].to_numpy() / np.var(agg_y))

np.save('generlization_curves_2026.npy', np.asarray(curves))
...
#         iterations=WL_ITERS,
#     )
#     perm_lbs.append(ev_perm.as_dataframe()["Lower Bound MSE"].to_numpy())
# perm_lbs = np.array(perm_lbs)  # (N_PERM_AGG, WL_ITERS+1)
# agg_perm_null = {
#     "mean": perm_lbs.mean(axis=0),
#     "std": perm_lbs.std(axis=0),
#     "n_iter": np.arange(perm_lbs.shape[1]),
#     "n_perm": N_PERM_AGG,
# }